...
public class TestAbstractCache {
	...
	public void testGetMaxCacheSize() {
		AbstractCacheWrapper uut = new AbstractCacheWrapper(entries, storageAll, IDataCache.STD_MAX_CACHE_SIZE, 
				predicate -> true);
	...
	public void testGetFreeSpace() {
		...
		uut.cacheData(c1);
		assertEquals(IDataCache.STD_MAX_CACHE_SIZE - (c0.getByteSize() + c1.getByteSize()), uut.getFreeSpace());	
	}
	...
	public void testUncacheData() {
		AbstractCacheWrapper uut = new AbstractCacheWrapper(entries, storageAll, 20000, predicate -> true);
		uut.uncacheData(c0);
	...
	public void testDataIsCachable() {
		...
		entries.clear();
		uut = new AbstractCacheWrapper(entries, storageAll, 20000, predicate -> predicate.getId() == c1.getId() 
				|| predicate.getId() == c3.getId());
		assertFalse(uut.dataIsCachable(c0));
		assertTrue(uut.dataIsCachable(c1));
		assertFalse(uut.dataIsCachable(c2));
		assertTrue(uut.dataIsCachable(c3));
	...
	public void testSetMaxCacheSize() {
		...
		AbstractCacheWrapper uut = new AbstractCacheWrapper(entries, storageAll, startSize, predicate -> true);
	...
	public void testCacheData() {
		...
		AbstractCacheWrapper uut = new AbstractCacheWrapper(entries, storageAll, 6500, predicate -> predicate != null);
		uut.cacheData(c0);
		assertTrue(uut.dataIsInCache(c0.getId()));
		uut.cacheData(c1);
		assertTrue(uut.dataIsInCache(c1.getId()));
		...
		// Test not caching items larger than the max cache size (should not modify already cached data)
		entries.clear();
